...
#  Last record on table east (for calendar min date)
try {
    $connection = new PDO($dsn, USERNAME, PASSWORD, $options);
    $connection->exec('SET NAMES utf8');
    $sql = 'SELECT added FROM `ilm_ds18_out_east` ORDER BY added ASC LIMIT 1';
    $statement = $connection->prepare($sql);
    $statement->execute();
    $east = $statement->fetch(PDO::FETCH_ASSOC);
    $firstdate = dbDateRemoveClock($east['added']);
    // show($east); # Testiks
...
try {
    ...
    for ($i = 0; $i < count($datestoread); $i++) {
        for ($j = 0; $j < count(TIME_FOR_TABLE); $j++) {
            $connection = new PDO($dsn, USERNAME, PASSWORD, $options);
            $connection->exec('SET NAMES utf8');
            $sql = 'SELECT celsius FROM ilm_ds18_out_east 
...
<div class="row justify-content-center mt-2">
    ...
    <?php } else { ?>
        ...
        <div class="col-auto m-2">
            ...
            <form action="east.php" method="post">
        ...
        <div class="col-auto m-2">
            <p class="fw-bold text-secondary mt-2">Temperatuurid idas kuupäeval: <?php echo (dbDateToEstDateOnly($selectedday)); ?></p>
